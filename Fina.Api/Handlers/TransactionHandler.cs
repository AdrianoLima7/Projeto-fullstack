...
public class TransactionHandler(AppDbContext context) : ITransactionHandler
{
    public async Task<Response<Transaction?>> CreateAsync(CreateTransactionRequest request)
    {
        ...
        try
        {
            var transaction = new Transaction
            {
                ...
                PaiOrReceivedAt = request.PaidOrReceivedAt,
                Title = request.Title,
                Type = request.Type
            };
    ...
    public async Task<PagedResponse<List<Transaction>?>> GetByPeriodAsync(GetTransactionsByPeriodRequest request)
    {
        ...
        try
        {
            var query = context
                ...
                .AsNoTracking()
                .Where(x =>
                x.PaiOrReceivedAt >= request.StartDate &&
                x.PaiOrReceivedAt <= request.EndDate &&
                x.UserId == request.UserId)
            .OrderBy(x => x.PaiOrReceivedAt);
        ...
        {
            return new PagedResponse<List<Transaction>?>(null, 500, "Não foi possível recuperar a transação");
        }
    }
    ...
    public async Task<Response<Transaction?>> UpdateAsync(UpdateTransactionRequest request)
    {
        ...
        try
        {
            ...
            transaction.PaiOrReceivedAt = request.PaidOrReceivedAt;
